...
module.exports = function (config, statsMod) {
    ...
    function serveUserStats() {
        ...
        router.get('/overview', (req, res) => {
            var interval = req.query.interval || '8';
            var queryStatName = req.query.statName || 'energyHarvested';
            ...
            var rooms = [], stats = {}, statsMax, totals = {}, gametimes = [];
            var baseStatArray = [];
            for (var i = blockStart; i <= endTime; i++) {
                baseStatArray.push({ endTime: i, value: 0 });
                gametimes.push(null);
            }
            ...
            .then((data) => {
                gametimes[0] = data; // main server only populates the first entry in the gametimes array
                storage.db.users.findOne(query).then((user) => {
                    if (!user) {
                        res.send({ error: "user not found" });
                        return;
                    }

                    storage.db['rooms.objects'].find({$and: [{ type: 'controller' }, { user: user._id }]})
                    .then((objects) => {
                        objects.sort((a, b) => b.level - a.level);
                        for (var obj of objects) {
                            rooms.push(obj.room);
                            stats[obj.room] = _.clone(baseStatArray);
                        }
                        return storage.db['rooms.stats' + interval].find({ $and: [{ endTime: { $gte: blockStart } }, { user: user._id }]});
                    }).then((rawStats) => {
                        var roomUserSums = {};
                        for (var statObj of rawStats) {
                            for (var statName of statsMod.statNames) {
                                totals[statName] = (totals[statName] || 0) + (statObj[statName] || 0);
                            }

                            if (statObj[queryStatName]) {
                                stats[statObj.room][statObj.endTime - blockStart].value += statObj[queryStatName];
                            }
                        }

                        return storage.db['rooms.stats.max'].find({ $and: [ { endTime: { $gte: blockStart } }, { interval } ] });
                    }).then((maxStats) => {
                        statsMax = 0;
                        for (var obj of maxStats) {
                            if (obj[queryStatName] > statsMax) statsMax = obj[queryStatName];
                        }
                    }).then(() => {
                        res.send({ ok: 1, rooms, stats, statsMax, totals, gametimes });
                    });
                });
            });
        });
    ...
    function serveExperimental() {
        ...
        router.get('/pvp', (req, res) => {
            ...
            .then((data) => {
                var time = data;
                storage.db.rooms.find({ lastPvpTime: { $gte: start } }).then((fightclub) => {
