...
module.exports = function (config) {
    ...
    return {
        ...
        // don't bother checking leaderboard enabling: it is just there for the sake of tools

        areStatsEnabled() {
            if (config.features) {
                return config.features.getFeatureEnabled('room-stats').then((val) => {
                    return val;
                });
            } else {
                return q.fcall(() => true);
        ...
        updateLeaderboardSeason() {
            ...
            return storage.db["leaderboard.seasons"].findOne({ _id: id }).then(result => {
                if (result) {
